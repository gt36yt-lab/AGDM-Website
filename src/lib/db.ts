...
export async function enrichCommentsWithStreaks(comments: Comment[]): Promise<{ comments: Comment[]; streaks: AccountStreakSummary[] }> {
  ...
  // Persist computed streaks to Supabase in the background (best-effort).
  // Disabled by default to avoid overwriting persisted values when callers
  // pass only a subset of comments (e.g. comments for a single quote).
  // To enable persistence explicitly set the environment variable
  // `PERSIST_STREAKS=true` in your deployment environment.
  if (process.env.PERSIST_STREAKS === 'true') {
    void (async () => {
      try {
        const client = getSupabaseClient();
        if (!client) return;
...
export async function setAccountStreak(userId: number | undefined, displayName: string, streak: number): Promise<boolean> {
  const client = getSupabaseClient();
  if (!client) return false;
  ...
  const saved = await upsertAccountStreakOverride(client, key, name, userId, streak);
  return saved;
}
...
export async function listAccountStreaks(): Promise<Array<Record<string, unknown>>> {
  ...
  const { data, error } = await client.from('account_streaks').select('*').order('updated_at', { ascending: false });
  if (error) {
    console.error('Supabase account_streaks list failed:', error);
    return [];
  }
